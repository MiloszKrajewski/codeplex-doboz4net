...
namespace DobozN
{
	public partial class DobozDecoder
	{
		...
		private static unsafe Result GetCompressionInfo(byte* source, int sourceSize, ref CompressionInfo compressionInfo)
		{
			...
			var headerSize = 0;
			var decodeHeaderResult = DecodeHeader(ref header, source, sourceSize, ref headerSize);
		...
		private static unsafe Result Decompress(byte* source, int sourceSize, byte* destination, int destinationSize)
		{
			fixed (LUTEntry* lut = &LUT[0])
			{
				...
				var src_end = src + ((header.compressedSize));
				var dst_end = dst + uncompressedSize;
				...
				var outputTail = (uncompressedSize > TAIL_LENGTH) ? (dst_end - TAIL_LENGTH) : dst;
		...
		private static unsafe Result DecodeHeader(ref Header header, byte* source, int sourceSize, ref int headerSize)
		{
			var src_p = (source);
